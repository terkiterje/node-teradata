...
Teradata = {
    connect: function (url, user, password) {
        ...
        return teradataInstance.initializeAsync()
            .then(function () {
                debug("Successfully initialized Teradata connection to %s ", teradataConfig.url);
                return teradataInstance.reserveAsync();
            })
            .then(function (teradataConnection) {
                debug("Teradata connected and ready for queries");
    ...
    disconnect: function () {
        return teradataInstance.releaseAsync(tdConn)
            ...
            .then(function (err) {
                debug("Teradata database disconnected");
            });
    },
...
var debug
if (process.env.NODE_DEBUG && /\bteradata\b/.test(process.env.NODE_DEBUG)) {
    debug = function() {
        var args = Array.prototype.slice.call(arguments)
        if (typeof args[0] === 'string') {
            args[0] = 'teradata: ' + args[0];
        } else {
            args.unshift('teradata:');
        }
        chalksay.green.apply(chalksay.green, args);
      }
} else {
    debug = function() {}
}
